...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Get the user identifier from environment variables
  const user = process.env.USER || 'default'; // Default to 'default' if not set
  
  // Redirect to the user-specific link with the path
  return res.redirect(307, `https://click2grow.in/link/${user}${path}`);
}
